...
- Seed script seeds two kinds of activity per venue, both surfaced in the merged Live Reports feed: 3–6 past-experience comments (deterministic pseudo-random authors/messages, createdAt spread 6h–40 days back) and 1–3 live reports (reporter handles like "NightOwl99", crowdLevel/waitTimeMinutes jittered off the venue's current stats, createdAt within the last ~10h so the feed reads as genuinely live). Pools are per-category plus a speakeasy pool for both comments and report vibe notes (matched via `bestFor.includes("Speakeasy")`). Venue insert uses `.returning()` for ids; comments/reports insert in chunks of 500.
...
- User accounts (Clerk): sign-in/sign-up at /sign-in and /sign-up; the watchlist bookmark persists to the account (`watchlist` table, /api/watchlist endpoints) when signed in and to localStorage when anonymous (local saves merge into the account on first sign-in); comments and live reports carry an `authorId`/`reporterId` when posted signed-in and are auto-attributed to the user's display name; /profile shows saved venues plus past reports and comments (/api/me/activity).
- Venue detail page: crowd score, wait time, headcount, line trend, seating odds, noise level, cover cost, best arrival window/timing strategy, a single merged "Live Reports" feed in the sidebar (structured pulse reports + past-experience comments combined, sorted by recency, with both a toggleable report form and an always-visible quick comment bar), watchlist bookmark, "Open Maps" link.
- Markets page (13 North American markets) and "For Operators" hospitality market-gap page.
- Seeded with 687 venues across 13 markets, including real named venues with source attribution (sourceLabel/sourceUrl: Google Maps listing, Yelp pick, Uber Eats/DoorDash/Grubhub favorite, Local favorite). Speakeasies (3–5+ per market) are surfaced in "The Speakeasy Files" scroller, the `/speakeasies` page, and the speakeasy intent filter.
- Browse-by-type system: `lib/venue-types.ts` defines VENUE_TYPES metadata (5 categories: bars/restaurants/cafes/retail/experiences) with tag-based subtypes matched case-insensitively against `bestFor` (incl. a GAME_TAGS set for game bars). Routes: `/types` index (category cards w/ live counts, `venue-type-card-{slug}`) and `/types/:slug` detail (`venue-type-detail.tsx`: breadcrumb via PageIntro parent, market filter Select `venue-type-market-filter`, 4-stat strip, expandable subtype sections `subtype-section-{type}-{subtype}` w/ 4-card preview + "Show all N" toggle, A–Z full directory w/ load-more, NotFound for bad slugs). Entry points: desktop nav "Types" link, mobile Sheet "Browse by type" section (`link-mobile-type-{slug}`), footer "Venue Types" link (`link-footer-types`), home `BrowseByType` strip (`browse-by-type`, after venue feed).
